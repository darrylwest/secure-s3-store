...
export class SecureS3Store {
    ...
    constructor(config) {
        ...
        if (!config.keys || Object.keys(config.keys).length === 0) {
            throw new ValidationError('At least one key must be provided in the `keys` configuration.');
        }
        if (!config.primaryKey || !config.keys[config.primaryKey]) {
            throw new ValidationError('The `primaryKey` must be a valid key identifier present in the `keys` configuration.');
        }
        this.keys = new Map();
        for (const kid in config.keys) {
            const secretKey = config.keys[kid];
            if (!/^[0-9a-fA-F]{64}$/.test(secretKey)) {
                throw new ValidationError(`Invalid secretKey for KID "${kid}": Must be a 64-character hex string.`);
            }
     ...
     */
    async list(path, offset = 0, limit = 1000, recursive = false) {
        ...
        try {
            ...
            this.logger.info(`Successfully listed ${allKeys.length} objects at path: ${path}`);
    ...
    validateInput(data) {
        ...
        if (data.length > this.maxFileSize) {
            throw new ValidationError(`File size of ${data.length} bytes exceeds maximum limit of ${this.maxFileSize} bytes`);
        }
    }
    ...
    static parsePath(path) {
        ...
        if (parts.length < 2) {
            throw new ValidationError('Invalid path format. Must be `bucket-name/key`.');
        }
        const bucket = parts[0];
        const key = parts.slice(1).join('/');
        if (!bucket || !key) {
            throw new ValidationError('Invalid path format. Bucket and key must not be empty.');
        }
        return { bucket, key };
    }
}
//# sourceMappingURL=SecureS3Store.js.map